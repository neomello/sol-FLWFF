...
const scrapeUrlForTag = async (url, tagName) => {
  const siteData = await fetch(url).catch((err) => {
    console.log(err);
  });
...
const getGHStargazers = async () => {
  const res = await fetch('https://api.github.com/repos/solana-foundation/solana-com', {
    method: 'GET',
    headers: {
...
const getYTVideos = async (
  ...
  if (!videosData.error) {
    ...
    videos.push(
      ...(videosData?.items?.filter((item) => Object.keys(item.snippet.thumbnails).length) || [])
    );
  } else {
