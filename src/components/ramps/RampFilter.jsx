  ...
  const toggleFilterStatus = (e) => {
    ...
    setOptionStates((prevOptionStates) =>
      prevOptionStates.map((option) =>
        option.value === filterValue && option.name === filterName ? { ...option, checked } : option
      )
    );
  ...
  return (
    <div className={`ramp-filter ${styles['ramp-filter']}`}>
      ...
      >
        ...
        <button
          aria-label={i18n['on-off-ramp.filters.mobile-dropdown-toggle.aria-label']}
      ...
      >
        <div>
          <input type="search" className={styles['ramp-filter__search-input']} value={filterText} />
        </div>

        <div className={inputType === 'currency' ? styles['ramp-options--currency'] : ''}>
          {filteredOptions.map((item, index) => (
            <div
              className={`${styles['ramp-options__option']} ${
                inputType === 'currency' ? styles['ramp-options__option--currency'] : ''
              }`}
              key={index}
            >
              <input
