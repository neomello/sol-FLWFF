...
export default function PodcastEpisode({ episode }) {
  ...
  const onSubscribeClick = useCallback(() => subscribeDialog.current?.open(), []);

  const duration = `${minutes}:${seconds}`;

  const authors = useMemo(() => {
    return episode.authors?.collection?.map((author) => author.name)?.join(', ');
  }, [episode]);
    ...
    <div className={classNames('d-flex flex-column', styles['episode'])}>
      <div className={styles['episode__header']}>
        ...
        <div>
          ...
          <span className={styles['episode__header--info']}>
            <FormattedDate date={episode.published_at} format="MMMM do, yyyy" /> | {duration}
      ...
      <div className={styles['episode__content']}>
        <div className={styles['episode__content--links']}>
          ...
          <a href="#" className="btn-sm btn-link p-0" onClick={onSubscribeClick} role="button">
        ...
        <div className={styles['episode__content--details']}>
          <div className="mb-4">
            ...
            <p className={styles['episode__content--details__text']}>{episode.description}</p>
          </div>

          <div className="mb-4">
            <h2 className={styles['episode__content--details__title']}>{podcast.episode.notes}</h2>
            <div
          ...
          {authors && (
            <>
              ...
              <p className={styles['episode__content--details__contributors']}>
                <span className="text-uppercase me-4">{podcast.episode.contributors}</span>
